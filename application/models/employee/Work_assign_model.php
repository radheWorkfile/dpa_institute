...
class Work_assign_model extends CI_Model
{
	...
	public function work_assign_query($where=false)   
	{
		...
		$this->db->where(array('rw.working_status'=>2, 'rw.status' => 2));
	...
	public function closed_work_query($where=false)   
	{
		...
		public function showWorkList_query($where=false)   
	    {
		$i=0;$field = array('id');
		foreach($field as $item)
		{
		...
		$this->db->select('rw.*,rw.created_at as startDate,emp.user_id as empId,emp.first_name as empName')->from('work_running as rw');
		if($where['work_id']){$this->db->where('rw.ass_work_id',$where['work_id']);}
		$this->db->where(array('rw.working_status'=>2, 'rw.status' => 1))->join('cordinator_manage as emp','emp.id=rw.emp_id');
		
		if(isset($where['order'])&& !empty($where['order'])){$this->db->order_by($field[$where['order']['0']['column']],$where['order']['0']['dir']);}else{$this->db->order_by('id','desc');
		}
		}  
		public function showWorkList($where = false,$accs=false)
		{$this->showWorkList_query($where,$accs);if ($where['length'] != -1) {$this->db->limit($where['length'], $where['start']);}return $this->db->get()->result();}
		public function showWorkList_total_count($accs=false){$this->showWorkList_query($where = false,$accs);return $this->db->get()->num_rows();}
		public function showWorkList_filter_count($where = false,$accs=false){$this->showWorkList_query($where,$accs);return $this->db->get()->num_rows();
		}
